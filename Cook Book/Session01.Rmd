...
library(dplyr)
library(readr)
data <- read_csv('BreastCancerData_Class01.csv')
```
...
dim shows us the dimensions of our data table
```{r}
data %>%
  dim()
```
Head shows us the top 6 rows of the data table
```{r}
data %>%
  head()
```

Tail shows us the bottom 6 rows of the data table
```{r}
data %>%
  tail()
```

str shows us the internal structure of the data table, so the column names and then a string of the the column entries, it also tells us what type of data is in each column ie. col_character and col_double
```{r}
data %>%
  str()
```

glimpse prints out our data table as a data frame, each row is a column and then the length of the row is all the entries for each column
```{r}
data %>%
  glimpse()
```

Summary gives you a the summary of each column, including things like how many entries there are (length) the class of the data, the mode, if it's numbers it will give you min, max, median, mean etc.
```{r}
data %>%
  summary()
```
...
```{r}
renamed_df <- data %>%
    rename(metastasis = metastasis.ch1, tissue = tissue.ch1, characteristics = characteristics_ch1)
renamed_df
```
